...
// make our own write trait to simplify writing to various outputs
...
impl HttpWriter {    
    fn new(http_output_url: &str, http_output_delimiter: Vec<u8>, http_output_include_delimiter: bool, http_output_rate: u64) -> Result<HttpWriter, Error> {
        // HTTP client init and configuration
        match url::Url::parse(http_output_url) {
            Ok(url) => {
                let output_rate = std::time::Duration::from_millis(http_output_rate);
                Ok(Self {
                    client: reqwest::Client::new(),
                    url,
                    output_delimiter: http_output_delimiter,
                    include_output_delimiter: http_output_include_delimiter,
                    output_rate: output_rate.clone(),
                    last_output: Instant::now() - output_rate, // backdate so we can write immediately after initialization
                    payload: Vec::new(),
                    buffer: Vec::new(),
                    buffer_index: 0,
                })
...
pub async fn get_output_writers(args: &ProgramArgs) -> Result<Vec<Writer>, Error> {
    ...
    if args.output.udp_output.is_some() {
     ...
     if args.output.http_output.is_some() {
            ...
            match HttpWriter::new(&url, http_output_delimiter: Vec<u8>, http_output_include_delimiter: bool, output_rate) {
            {
                Ok(http_writer) => {
